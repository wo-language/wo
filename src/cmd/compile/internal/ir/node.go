...
const (
    ...
    ORARROW    // Type -> Func.Closure.Body #wo
    ...
    ODCLSHADOW    // X; X := Y #wo
    OTAGS         // <Type{List}> #wo
    ODEFAULTFIELD // X=Y X.Type #wo
    OUNWRAPPANIC  // Errable[T]!! - if T is err panic else T #wo
    ...
    // if X then Y else Z // returns Y or Z #wo
    // if var X = Y? { } #wo
    // try var X = Y! { } #wo
    OLABEL       // Label:
    OGO          // go Call
    ORANGE       // for Key, Value = range X { Body
    OENHANCEDFOR // for Key, Value : X { Body } or for Value : X #wo
