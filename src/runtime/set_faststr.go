...
func setdelete_faststr(t *settype, h *hset, ky string) {
	if raceenabled && h != nil {
		callerpc := getcallerpc()
		racewritepc(unsafe.Pointer(h), callerpc, abi.FuncPCABIInternal(setdelete_faststr))
	}
	if h == nil || h.count == 0 {
		return
	}
...
func growWorkSet_faststr(t *settype, h *hset, bucket uintptr) {
	// make sure we evacuate the oldbucket corresponding
	// to the bucket we're about to use
	evacuateSet_faststr(t, h, bucket&h.oldbucketmask())

	// evacuate one more oldbucket to make progress on growing
	if h.growing() {
		evacuateSet_faststr(t, h, h.nevacuate)
	}
}

func evacuateSet_faststr(t *settype, h *hset, oldbucket uintptr) {
