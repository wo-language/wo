...
func setaccess1_fast64(t *settype, h *hset, key uint64) unsafe.Pointer {
	if raceenabled && h != nil {
		callerpc := getcallerpc()
		racereadpc(unsafe.Pointer(h), callerpc, abi.FuncPCABIInternal(setaccess1_fast64))
	}
	if h == nil || h.count == 0 {
...
// setaccess2_fast64 should be an internal detail,
...
func setaccess2_fast64(t *settype, h *hset, key uint64) (unsafe.Pointer, bool) {
	if raceenabled && h != nil {
		callerpc := getcallerpc()
		racereadpc(unsafe.Pointer(h), callerpc, abi.FuncPCABIInternal(setaccess2_fast64))
	}
	if h == nil || h.count == 0 {
...
// setassign_fast64 should be an internal detail,
// but widely used packages access it using linkname.
// Notable members of the hall of shame include:
...
func setdelete_fast64(t *settype, h *hset, key uint64) {
	if raceenabled && h != nil {
		callerpc := getcallerpc()
		racewritepc(unsafe.Pointer(h), callerpc, abi.FuncPCABIInternal(setdelete_fast64))
	}
	if h == nil || h.count == 0 {
		return
	}
...
func growWorkSet_fast64(t *settype, h *hset, bucket uintptr) {
	// make sure we evacuate the oldbucket corresponding
	// to the bucket we're about to use
	evacuateSet_fast64(t, h, bucket&h.oldbucketmask())

	// evacuate one more oldbucket to make progress on growing
	if h.growing() {
		evacuateSet_fast64(t, h, h.nevacuate)
	}
}

func evacuateSet_fast64(t *settype, h *hset, oldbucket uintptr) {
